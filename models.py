...
class GovernmentManager(models.Manager):
    ...
    def get_government(self, schoolyear = None):
        ...
        government_of_schoolyear = self.get_query_set().filter(schoolyear=schoolyear)
        return government_of_schoolyear.exclude(role__is_president = False) | government_of_schoolyear.exclude(role__is_president = True) 
        #Now the presidental roles are always first in the query
